...
export default function Registrar() {
  ...
  async function aoEnviar(e: React.FormEvent) {
    ...
    setCarregando(true);
    try {
      await registrar(email.trim(), nomeJogador.trim(), senha);
      navigate("/");
    } catch (err) {
      setErro(mensagemDeErro(err, "Nao foi possivel criar sua conta. Tente novamente."));
    } finally {
      setCarregando(false);
    }
  }
  ...
  return (
    <div className="min-h-dvh flex flex-col items-center justify-center px-6 py-10 gap-6">
      ...
      >
        <h1 className="titulo-jogo text-2xl text-ouro-400 text-center">Criar conta</h1>
        ...
        <div className="flex flex-col gap-1">
          <label className="text-xs text-creme/60">Senha (minimo 6 caracteres)</label>
          <input
            type="password"
            value={senha}
        ...
        <p className="text-sm text-center text-creme/60">
          Ja tem conta?{" "}
          <Link to="/login" className="text-ouro-400 hover:underline">
            Entrar
          </Link>
        </p>
      </form>
